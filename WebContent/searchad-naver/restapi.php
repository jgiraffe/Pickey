<?php
	class SearchAdRestApi
	{
		protected $LOCAL_PATH = "searchad-naver/";
		...
		protected function build_http_query($query)
		{
			...
			$json_decoded = json_decode($query);
				foreach ($json_decoded as $key => $key_value) {
		...
		public function WRITE_FILE($response = array())
		{
			$fp = fopen($this->LOCAL_PATH."result.json", "w+");
		...
		{
			$fp = fopen($this->LOCAL_PATH."result_download", "w");
			$ch = curl_init();
